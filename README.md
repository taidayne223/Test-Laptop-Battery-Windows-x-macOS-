...
> 🧯 **Về lỗi YouTube hay bị "đơ" / đen màn hình:** YouTube có cơ chế tự phát video tiếp theo, đôi khi nhảy vào đoạn gần hết video rồi treo hoặc kẹt ở màn hình loading. Công cụ đã xử lý bằng cách **hard-refresh sau khi bật chế độ rạp phim**, **tự tua video về đầu** (nhấn phím `0`) và **hard-refresh trang định kỳ** trong lúc xem.
...
| `youtube_test.hard_reload_after_theater_mode_enabled` | Hard-refresh ngay sau khi bấm `T` bật chế độ rạp phim, dùng để né lỗi YouTube đen màn hình |
