...
- **Chống YouTube autoplay-freeze:** YouTube tự phát video kế tiếp, đôi khi nhảy vào đoạn gần hết rồi treo. Giải pháp: nhấn `0` để tua về đầu lúc bắt đầu, và **re-seek định kỳ** mỗi `youtube_test.reseek_interval_seconds` giây bằng cách chia nhỏ `sleep` → video không bao giờ tới đoạn cuối. Tổng thời gian xem mỗi video được giữ nguyên. Đặt `reseek_interval_seconds: 0` để tắt.
...
## 6. Thay đổi gần đây (phiên 2026-06-22)

1. **youtube_test.py** — thêm click lấy focus sau hard-refresh; thay `sleep` dài bằng vòng lặp re-seek `0` định kỳ chống autoplay-freeze.
2. **config.py + benchmark_config.json** — thêm `youtube_test.reset_to_start_enabled` và `youtube_test.reseek_interval_seconds` (mặc định 60).
