...
4. `generate_report()` dựng báo cáo ngắn theo từng session: thời điểm test, model, pin specs/thực tế, độ chai và tổng phút; cuối file có TSV để dán database/Google Sheets. Lưu `battery_report.txt`.
...
- **YouTube hotkey chỉ ăn khi page/player có focus.** Trong `youtube_test.py`, phải click lấy focus **trước** khi hard-refresh (`Ctrl/Cmd+Shift+R` / `Cmd+Shift+R`). Không click lại sau reload vì cú click đó có thể pause video; sau reload nhấn `0` trực tiếp.
- **Chống YouTube autoplay-freeze / black loading screen:** YouTube tự phát video kế tiếp, đôi khi nhảy vào đoạn gần hết rồi treo, hoặc bị kẹt màn hình đen/loading. Giải pháp: hard-refresh lúc setup, nhấn `0` để tua về đầu, **re-seek định kỳ** mỗi `youtube_test.reseek_interval_seconds` giây, và **hard-refresh định kỳ** theo `youtube_test.auto_hard_reload_interval_seconds` với giới hạn `youtube_test.max_auto_hard_reloads`. Đặt `reseek_interval_seconds: 0` hoặc `auto_hard_reload_interval_seconds: 0` để tắt từng cơ chế.
- **Popup pin yếu trên Windows:** `utils/low_battery_popup.py` theo dõi cửa sổ foreground khi pin xuống dưới `low_battery_popup.threshold_percent`. Chỉ khi UI Automation tìm thấy đúng câu `Your battery is running low` thì code mới gửi Enter trực tiếp vào cửa sổ đó, tránh làm pause YouTube do bấm Enter mù.
...
1. **youtube_test.py** — giữ flow YouTube đơn giản: mở video, click lấy focus, hard-refresh, nhấn `0` nếu cần, vòng lặp re-seek `0` định kỳ chống autoplay-freeze, và hard-refresh định kỳ trong lúc xem để thoát YouTube black/loading screen.
2. **config.py + benchmark_config.json** — giữ `youtube_test.reset_to_start_enabled`, `youtube_test.reseek_interval_seconds` (mặc định 60), `youtube_test.auto_hard_reload_interval_seconds` (mặc định 120) và `youtube_test.max_auto_hard_reloads` (mặc định 2).
